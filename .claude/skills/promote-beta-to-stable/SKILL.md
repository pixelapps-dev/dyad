...
Promote the latest pre-release of pixelapps-dev/dyad to a stable release.
...
1. **Look up the latest pre-release:**
   ...
   ```
   gh release list --repo pixelapps-dev/dyad --limit 10 --json tagName,isPrerelease
   ```
...
7. **Push the branch to upstream (pixelapps-dev/dyad):**
