...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("pagemate", process.execPath, [
      path.resolve(process.argv[1]),
    ]);
  }
} else {
  app.setAsDefaultProtocolClient("pagemate");
}
...
export async function onReady() {
  ...
  if (settings.enableNativeGit) {
    ...
    // the user starts interacting with Pagemate and uses a git-related feature.
  ...
  if (settings.enableAutoUpdate) {
    ...
    const host = `https://api.pagemate.dev/v1/update/${postfix}`;
...
async function handleDeepLinkReturn(url: string) {
  // example url: "pagemate://supabase-oauth-return?token=a&refreshToken=b"
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (parsed.protocol !== "pagemate:") {
    dialog.showErrorBox(
      "Invalid Protocol",
      `Expected pagemate://, got ${parsed.protocol}. Full URL: ${url}`,
    );
    return;
  }
  ...
  // pagemate://dyad-pro-return?key=123&budget_reset_at=2025-05-26T16:31:13.492000Z&max_budget=100
  ...
  // pagemate://add-mcp-server?name=Chrome%20DevTools&config=eyJjb21tYW5kIjpudWxsLCJ0eXBlIjoic3RkaW8ifQ%3D%3D
  ...
  // pagemate://add-prompt?data=<base64-encoded-json>
